...
import { updateInventaris } from "@/lib/actions/inventaris-actions";

interface EditButton extends ButtonProps {
  id: string;
  nama: string;
  jumlah: number;
  keterangan?: string;
...
export function EditButton({
  ...
  async function onSubmit(data: FormData) {
    const res = await updateInventaris(id, data);
    if (res.error) {
      return toast({
    ...
    toast({
      title: "Berhasil",
      description: "Inventaris berhasil diubah",
    });
    router.refresh();
  }
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Silahkan isi form berikut untuk mengubah inventaris
          </DialogDescription>
        </DialogHeader>
